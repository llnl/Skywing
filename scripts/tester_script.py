...
TURN_RED = "\033[585;91m"
RESET_TEXT = "\033[585;0m"
NUM_TRIES = 3
...
with open(sys.argv[1]) as f:
    ...
    def parse_line(line):
        ...
        return {"name": split_line[0], "command": split_line[1:]}
...
for i, command in enumerate(commands):
    ...
    for attempt_no in range(NUM_TRIES):
        print(
            f'Running "{command["name"]}"; {attempt_no + 1:>{NUM_TRIES_LEN}}/{NUM_TRIES:>{NUM_TRIES_LEN}}',
            end="",
            flush=True,
        )
        try:
