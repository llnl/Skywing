...
class PushSumProcessor(Processor):
    ...
    def convert(self, publish_data: PushSumData):
        # WM: only doing for float here, since these functions are going away
        return [], [publish_data.sigma_x, publish_data.sigma_y], []

    def deconvert(self, recv_strings, recv_doubles, recv_ints):
        # WM: only doing for float here, since these functions are going away
        if len(recv_doubles):
            return PushSumData(sigma_x=recv_doubles[0], sigma_y=recv_doubles[1])
        else:
            return ()
