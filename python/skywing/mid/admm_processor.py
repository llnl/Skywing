...
class ADMMProcessor(Processor):
    ...
    def convert(self, send_data: ADMMData):
        """Dict keys (tags) are sent as list of strings, the dict values (np.arrays) are
        concatenated together and sent as a long list of doubles, the integer iteration
        count is sent as a list of length one."""
        send_strings = []
        send_doubles = []
        send_ints = [send_data.my_iter]
        for _tag in send_data.q_j:
            send_strings.append(_tag)
            send_doubles += list(send_data.q_j[_tag])

        return send_strings, send_doubles, send_ints

    def deconvert(self, recv_strings, recv_doubles, recv_ints):
        """Deconvert back to dict of q_j's indexed by tag and nbr iteration count."""
        nbr_dict = {}
        nbr_iter = -1
        i = 0
        for _tag in recv_strings:
            nbr_dict[_tag] = np.array(recv_doubles[i : i + len(self.v)])
            nbr_iter = recv_ints[0]
            i += len(self.v)
        return ADMMData(q_j=nbr_dict, my_iter=nbr_iter)
