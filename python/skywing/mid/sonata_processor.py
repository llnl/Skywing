...
class SONATAProcessor(Processor):
    ...
    def convert(self, publish_data: SONATAData):
        double_list = []
        if len(publish_data.v):
            double_list += list(publish_data.rho)
            double_list += list(publish_data.v)
        return [], double_list, []

    def deconvert(self, recv_strings, recv_doubles, recv_ints):
        if len(recv_doubles):
            half = int(len(recv_doubles) / 2)
            rho = np.array(recv_doubles[:half])
            v = np.array(recv_doubles[half:])
            return SONATAData(rho=rho, v=v)
        else:
            return {}
