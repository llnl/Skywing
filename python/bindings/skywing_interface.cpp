...
/* Summation jobs */
using CountProcessor =
  QUACCProcessor<BigFloat, MinProcessor<BigFloat>, PushFlowProcessor<BigFloat>>;

template<typename val_t>
using SumMethod =
  SumProcessor<val_t, PushFlowProcessor<val_t>, CountProcessor>;
...
void bind_associative_vector(py::module_& m, const char* name) {
  py::class_<StandardVector<val_t>>(m, name)
    .def(py::init<>())
    .def("__getitem__", &StandardVector<val_t>::operator[])  // Bind the getter
...
void bind_ConsensusPythonJob(py::module_& m, const char* name) {
  py::class_<ConsensusPythonJob>(m, name, py::dynamic_attr())
    .def(py::init<py::object, int, std::string, std::vector<std::string>&&, size_t>())
...
void bind_Manager(py::module_& m, const char* name) {
  py::class_<Manager>(m, name)
    ...
    .def("configure_initial_neighbors",
	 [](Manager &self, std::string address, std::uint16_t port,
	    int timeout = 10) {
	   self.configure_initial_neighbors(address, port,
					    std::chrono::seconds(timeout));
	 },
	 py::arg("address"), py::arg("port"), py::arg("timeout") = 10);
}
