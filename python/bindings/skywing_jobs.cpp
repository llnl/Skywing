...
public:
  ...
  bool is_waiter_finished()
  { return waiter_finished_; }

  void submit_to_manager(Manager& manager, std::string& job_name)
  {
    auto job_fun = [&](skywing::Job& job, skywing::ManagerHandle manager_handle)
    {
	...
	WaiterBuilder<IterMethod>(manager_handle, job, this_id_, sum_sub_tags_)
	.set_processor(input_value_)
	.set_publish_policy()
	.set_stop_policy(std::chrono::seconds(run_duration_secs_))
...
{
public:
  using result_t = std::tuple<std::vector<std::string>, std::vector<double>, std::vector<int>>;
  ...
  void submit_to_manager(Manager& manager, std::string& job_name)
  {
    auto consensus_job_fun = [&](skywing::Job& job, skywing::ManagerHandle manager_handle)
    {
