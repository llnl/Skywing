...
{
public:
    explicit Continuation(Waiter wait_on,
                          Continuations... continuations) noexcept
    ...
    template <class Rep, class Period>
    bool
    wait_until(const std::chrono::time_point<Rep, Period>& end_time) noexcept
    {
...
private:
    ...
    decltype(auto) handle_continuations(const BuildUp& build_up,
                                        Next& next,
                                        Rest&... rest) noexcept
    {
        const auto next_call = [&]() noexcept -> decltype(auto) {
            // Tried to make this it's own function to make this tidier but it
            // didn't work for some reason?  So some repetition is unavoidable
            // it seems Can't pass void so have to make a wrapper
 ...
 * 1. The <em>instant Waiter</em> that is immediately ready. Enables lazy
 * construction of objects.
 * 2. The <em>void Waiter</em> which does not return anything upon @p get().
 * Enables checking for completion of tasks without needing to build an
 * associated object.
 *
...
public:
    ...
    /** @param mutex_handle A reference to the mutex used with the associated
     * condition variable.
     *  @param cv_handle A reference to the condition variable to wait on.
     *  @param is_ready_callable An @p std::function<bool()> that must return
     * true to declare readiness.
     *  @param get_value_callable An @p std::function<T()> that returns the
     * object of interest.
     */
    ...
    ValueType get() noexcept
    {
        ...
        if (!is_ready_no_lock()) {
            (*cv_)->wait(lock,
                         [this]() noexcept { return is_ready_no_lock(); });
        }
        return get_value_callable_();
    }
    ...
    /** @brief Block until ready or a given amount of time passes, whichever is
     * first.
     */
    template <class Rep, class Period>
    bool wait_for(const std::chrono::duration<Rep, Period>& wait_time) noexcept
    ...
    wait_until(const std::chrono::time_point<Rep, Period>& end_time) noexcept
    {
        ...
        return (*cv_)->wait_until(
            lock, end_time, [this]() noexcept { return is_ready_no_lock(); });
    }
    ...
    Continuation<Waiter, Continuations...>
    then(Continuations... continuations) && noexcept
    {
        return Continuation<Waiter, Continuations...>{
            *this, std::move(continuations)...};
    }
...
public:
    ...
    // TODO: Will this correctly perform lazy construction of an ObjecT with
    // perfect forwarding?
    ...
    template <typename... Args>
    WaiterBuilder(Args&&... args)
        : waiter_([&]() { return ObjectT(std::forward<Args>(args)...); })
    {}
...
{
    return Waiter<T>{
        mutex, cv, std::move(is_ready_callable), std::move(get_value_callable)};
}

inline Waiter<void>
make_waiter(std::mutex& mutex,
            std::condition_variable& cv,
            std::function<bool()>&& is_ready_callable) noexcept
{
    return Waiter<void>(
        mutex, cv, std::move(is_ready_callable), WaiterGetNoOp{});
}
...
public:
    ...
    explicit WaiterVec(std::vector<Waiter<T>> waiters) noexcept
