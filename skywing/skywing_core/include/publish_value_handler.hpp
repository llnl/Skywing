...
{
// For recursing below, I feel like there's a better way of doing this, but I
// can't think of it.
template <typename T>
...
std::vector<To> list_to_vector(const From& values) noexcept
{
    ...
    for (std::size_t i = 0; i < values.size(); ++i) {
        if constexpr (IsVector<To>::value) {
            to_ret.push_back(
                list_to_vector<typename To::value_type>(values[i]));
        }
        else {
...
#define SKYNET_MAKE_PUBLISH_VALUE_HANDLER(cpp_type, capn_suffix)               \
    ...
    {                                                                          \
        static std::optional<cpp_type>                                         \
        get(const cpnpro::PublishValue::Reader& r) noexcept                    \
        {                                                                      \
            if (!r.is##capn_suffix()) {                                        \
                return {};                                                     \
            }                                                                  \
        ...
        static void set(cpnpro::PublishValue::Builder& b,                      \
                        const cpp_type& value) noexcept                        \
        {                                                                      \
    ...
    {                                                                          \
        static std::optional<std::vector<cpp_type>>                            \
        get(const cpnpro::PublishValue::Reader& r) noexcept                    \
        {                                                                      \
            if (!r.isR##capn_suffix()) {                                       \
                return {};                                                     \
            }                                                                  \
            return list_to_vector<cpp_type>(r.getR##capn_suffix());            \
        }                                                                      \
        static void set(cpnpro::PublishValue::Builder& b,                      \
                        const std::vector<cpp_type>& values) noexcept          \
        {                                                                      \
...
struct PublishValueHandler<std::vector<std::byte>>
{
    ...
    static void set(cpnpro::PublishValue::Builder& b,
                    const std::vector<std::byte>& values) noexcept
    {
