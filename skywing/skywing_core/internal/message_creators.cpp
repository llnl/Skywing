...
namespace
{
// Creates a vector of bytes with a size pre-prended ready to be sent over the
// network
std::vector<std::byte>
finalize_message(capnp::MallocMessageBuilder& builder) noexcept
{
    // Calculate the sizes
    ...
    const std::size_t msg_size =
        capnp::computeSerializedSizeInWords(builder) * 8 + 8;
#else
    const std::size_t msg_size =
        capnp::computeSerializedSizeInWords(builder) * sizeof(std::size_t);
#endif
    ...
    kj::Array<kj::byte> buffer{reinterpret_cast<kj::byte*>(buffer_data.data())
                                   + net_size,
...
void set_publish_data(cpnpro::PublishData::Builder to_set,
...
void set_vector(const InitFunc& init_func,
                MessageType msg,
                const std::vector<VecValueType>& values) noexcept
{
...
std::vector<std::byte> make_remove_neighbor(const MachineID& neighbor) noexcept
{
    capnp::MallocMessageBuilder builder;
    auto message =
        builder.initRoot<cpnpro::StatusMessage>().initRemoveNeighbor();
    message.setNeighborID(neighbor);
    return finalize_message(builder);
}
...
{
    const auto set_nested_vector = [&](auto builder,
                                       const auto& set_to) noexcept {
...
make_get_publishers(const std::vector<TagID>& tags,
                    ...
                    const bool ignore_cache) noexcept
{
    assert(tags.size() == publishers_needed.size());
    capnp::MallocMessageBuilder builder;
    auto message =
        builder.initRoot<cpnpro::StatusMessage>().initGetPublishers();
    set_vector(&decltype(message)::initTags, message, tags);
    set_vector(
        &decltype(message)::initPublishersNeeded, message, publishers_needed);
                                                ...
                                                bool is_unsubscribe) noexcept
{
    capnp::MallocMessageBuilder builder;
    auto message =
        builder.initRoot<cpnpro::StatusMessage>().initSubscriptionNotice();
    set_vector(&decltype(message)::initTags, message, tags);
