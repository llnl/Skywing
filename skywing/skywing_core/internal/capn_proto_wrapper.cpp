...
std::optional<std::vector<PublishValueVariant>>
PublishData::value() const noexcept
{
    using namespace detail;
    const auto decode_value = [](cpnpro::PublishValue::Reader reader)
        -> std::optional<PublishValueVariant> {
...
VersionID PublishData::version() const noexcept
{
    return r.getVersion();
}
TagID PublishData::tag_id() const noexcept
{
    return r.getTagID();
}
...
MachineID Greeting::from() const noexcept
{
    return r.getFrom();
}
...
MachineID NewNeighbor::neighbor_id() const noexcept
{
    return r.getNeighborID();
}
NewNeighbor::NewNeighbor(cpnpro::NewNeighbor::Reader reader) noexcept
    : r{std::move(reader)}
{}
...
MachineID RemoveNeighbor::neighbor_id() const noexcept
{
    return r.getNeighborID();
}
RemoveNeighbor::RemoveNeighbor(cpnpro::RemoveNeighbor::Reader reader) noexcept
    : r{std::move(reader)}
{}
...
std::vector<TagID> ReportPublishers::tags() const noexcept
{
...
ReportPublishers::ReportPublishers(
    cpnpro::ReportPublishers::Reader reader) noexcept
    : r{std::move(reader)}
{}
...
std::vector<std::string> GetPublishers::tags() const noexcept
{
    return detail::list_to_vector<TagID>(r.getTags());
}
...
bool GetPublishers::ignore_cache() const noexcept
{
    return r.getIgnoreCache();
}
...
bool SubscriptionNotice::is_unsubscribe() const noexcept
{
    return r.getIsUnsubscribe();
}
    ...
    -> std::optional<MessageVariant>
{
    ...
    const std::optional<MessageVariant> to_ret =
        [&]() -> std::optional<MessageVariant> {
