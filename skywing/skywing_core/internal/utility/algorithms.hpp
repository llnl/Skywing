 ...
 */
template <typename T>
auto merge_impl(T& lhs,
                T& rhs,
                PriorityTag<1>) noexcept -> decltype((void) lhs.merge(rhs))
{
    lhs.merge(rhs);
}
...
std::vector<std::common_type_t<typename Ts::value_type...>>
concatenate(const Ts&... containers) noexcept
{
    using ::std::begin;
    using ::std::cbegin;
    using ::std::cend;
    ...
    std::vector<std::common_type_t<typename Ts::value_type...>> to_ret(
        (size(containers) + ...));
 ...
 *
 * This is only needed as a workaround for when the merge method isn't
 * supported.
 */
template <typename T>
void merge_associative_containers(T& lhs, T& rhs) noexcept
{
    return ::skywing::internal::detail::merge_impl(
        lhs, rhs, detail::PriorityTag<1>{});
}
...
split(const std::string& to_split,
      const char split_char,
      const std::size_t max_count = 0) noexcept
{
    std::vector<std::string_view> to_ret;
    auto start_pos = to_split.find(split_char);
    auto last_pos = 0;
    for (auto pos = start_pos; pos != std::string::npos;
         pos = to_split.find(split_char, pos + 1))
    {
        // Add one as there's always one string appended at the end
        // This also makes a max_count of 0
        if (to_ret.size() + 1 == max_count) {
            break;
        }
        // to_ret.emplace_back(to_split.data() + last_pos, to_split.data() +
        // pos);
...
public:
    ...
    constexpr std::tuple<Iters...> underlying_iters() const noexcept
    {
        return iters_;
    }
    ...
    {
        return for_each_iter(
            [](auto& iter) noexcept -> decltype(auto) { return *iter; });
    }
    constexpr const reference& operator*() const noexcept
    {
        return for_each_iter(
            [](const auto& iter) noexcept -> decltype(auto) { return *iter; });
    }
    ...
    friend constexpr bool operator==(const ZippedIterEqualLength& lhs,
                                     const ZippedIterEqualLength& rhs) noexcept
    {
...
private:
    ...
    {
        return for_each_iter_impl(std::forward<Callable>(c),
                                  std::index_sequence_for<Iters...>{});
    }

    template <typename Callable, std::size_t... Is>
    constexpr auto for_each_iter_impl(Callable&& c,
                                      std::index_sequence<Is...>) noexcept
    {
        // make_tuple will remove references, which we don't want
        // this is so ugly though...
        return std::tuple<decltype(wrap_void_func(c, std::get<Is>(iters_)))...>{
            wrap_void_func(c, std::get<Is>(iters_))...};
    }
 ...
 *
 * Regardless of the underlying iterator types, the returned iterator is always
 * a forward read-only iterator
 */
template <typename... Iters>
