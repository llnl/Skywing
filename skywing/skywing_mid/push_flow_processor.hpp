...
public:
                        ...
                        const IterMethod& iter_method)
    {
        ...
        for (const auto& nbr_val : f_ij_num_)
            curr_num_ -= nbr_val.second;
        curr_denom_ = my_weight_;
        for (const auto& nbr_val : f_ij_denom_)
            curr_denom_ -= nbr_val.second;
    }
