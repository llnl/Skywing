#ifndef SKYWING_MID_ASYNCHRONOUS_ITERATIVE_HPP
#define SKYWING_MID_ASYNCHRONOUS_ITERATIVE_HPP
...
/** @brief A template specialization of WaiterBuilder for AsynchronousIterative
 * methods.
 *
...
public:
    ...
    ThisT& set_publish_policy(Args&&... args)
    {
        publish_policy_waiter_ = std::make_shared<Waiter<PublishPolicy>>(
            WaiterBuilder<PublishPolicy>(std::forward<Args>(args)...)
                .build_waiter());
        return *this;
    }
...
#endif // SKYWING_MID_ASYNCHRONOUS_ITERATIVE_HPP
