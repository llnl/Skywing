...
namespace skywing
{
/** @brief Communication-Efficient Decentralized Linear Learning (COLA) for least squares
 *
 ...
 */
template <typename index_t, typename scalar_t = double, typename tag_t = std::string>
class COLAProcessor
{
public:
    ...
    /**
     * @param A_k_ stores the matrix columns associated with this process (note the matrix is still stored row-wise)
     *        A_k is (d x n_k), that is d rows (keys) each of length n_k
     * @param b_ is length d rhs vector
     * @param W_k_ is a vector determining neighbor contributions in the communication step
     * @param x_k_ is a vector of lenght n_k with keys corresponding to column indices of A_k_
     ...
     */
    COLAProcessor(AssociativeMatrix A_k, ClosedVector b, AssociativeVector<tag_t, scalar_t, false> W_k, scalar_t lambda, index_t K, bool shift_scale = true)
        : A_k_(A_k),
          b_(b),
          W_k_(W_k),
          v_k_(A_k.get_keys()),
          v_k_prev_(A_k.get_keys()),
          lambda_(lambda),
          K_(K),
          shift_scale_(shift_scale)
    {
        ...
        // Setup Eigen matrix for the local least-squares problem: M = [A_k_; lambda_ * I]
        // Setup mapping from row keys to integer row indices
                        ...
                        const IterMethod&)
    {
        ...
        rel_norm_delta_ = sqrt(delta_x_k_.dot(delta_x_k_)) / sqrt(x_k_.dot(x_k_));
    ...
    ClosedVector get_local_solution() const {
        return x_k_;
    }

    ClosedVector get_local_v_k() const {
        return v_k_;
    }

    scalar_t get_rel_norm_delta() const {
        return rel_norm_delta_;
    }
    ...
    scalar_t compute_duality_gap() const {
        ...
        for (const auto& row_key : A_k_.get_keys()) {
            for (const auto& col_key : A_k_.at(row_key).get_keys()) {
                duality_gap += (1. / (2. * lambda_ * K_ * K_)) * A_k_.at(row_key).at(col_key) * v_k_minus_b.at(row_key);
            }
        }
        return duality_gap;
    }
...
private:
    ...
    {
        // Setup the Eigen vector for the right-hand side of the local least-squares problem: b_ls = (b - v_k)/K
        Eigen::VectorXd b_ls(M_k_.rows());
        for (const auto& row_key : A_k_.get_keys()) {
            b_ls(row_map_[row_key]) = (double) ((b_[row_key] - v_k_[row_key]) / K_);
        }
