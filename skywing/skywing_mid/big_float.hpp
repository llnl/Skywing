...
private:
    ...
    {
        // the fraction parameter may not be in the correct range, so need to
        // adjust.
        int exp_from_frac;
...
BigFloat operator+(const BigFloat& a, const BigFloat& b)
{
    ...
    if (mag_diff > 0) {
        ...
        return BigFloat(a.exp_, a.fraction_ + b.fraction_);
}
...
{
    return BigFloat(a.exp_, -a.fraction_);
}
...
{
    if (a.fraction_ == 0)
        return (0 < b.fraction_);
    if (b.fraction_ == 0)
        return (a.fraction_ < 0);
    if (a.exp_ == b.exp_)
        return (a.fraction_ < b.fraction_);
    if ((a.fraction_ * b.fraction_) < 0)
        return a.fraction_ < b.fraction_;
...
BigFloat pow2(const BigFloat& input)
{
    ...
    if (std::abs(input.exp_) > MAX_EXP)
        throw std::runtime_error("BigFloat::pow2: Input BigFloat is too large, "
                                 "will overflow even BigFloat.");
    ...
    std::int64_t new_exp =
        new_base.exp_ * two_exp; // (2^p)^(two_exp) = 2^(two_exp * p)
...
BigFloat log2(const BigFloat& input)
{
    ...
    return BigFloat(static_cast<double>(input.exp_)
                    + std::log2(input.fraction_));
}
