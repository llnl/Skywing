...
struct PubSubConverter
{
    static_assert(
        ...
        "Looks like you want Skywing to send messages with a new data type. "
        "For Skywing to do this, you must implement a specialization of "
        "PubSubConverter<T> to translate it into something Skywing knows how "
        "to send. See skywing/skywing_mid/pubsub_converter.hpp for more "
        "information.");
 ...
 * using MyPSTuple = PubSub_t<MyTuple>; // has type std::tuple<double, unsigned,
...
struct PubSubConverter<std::tuple<Ts...>>
{
    ...
    using pubsub_type = decltype(std::tuple_cat(
        std::declval<
            typename TupleIfNotAlready<PubSub_t<Ts>>::tuple_type>()...));

    /** @brief Convert an input of type \c input_type into a tuple of type \c
     * pusub_type.
     *
     ...
     * Then we have
     * \code{.cpp}
     * std::tuple<PS1, PS21, PS22, PS3> = PubSubConverter<std::tuple<T1, T2,
     * T3>>::pubsub_type \endcode
     *
     * Now suppose we have a runtime \c my_tup of type \c std::tuple<T1, T2,
     * T3>. Then we have \code{.cpp} using pubsub_t = typename
     * PubSubConverter<std::tuple<T1, T2, T3>>::pubsub_type; pubsub_t my_ps_tup
     * = PubSubConverter<std::tuple<T1, T2, T3>>::convert(my_tup); \encode
     */
    ...
    /** @brief Convert an input of type \c pubsub_type into something of type \c
     * input_type.
     *
     ...
     * agent. Then we have
     * \code{.cpp}
     * using orig_type = typename PubSubConverter<std::tuple<T1, T2,
     * T3>>::input_type; // is type std::tuple<T1, T2, T3> orig_t my_tup =
     * PubSubConverter<std::tuple<T1, T2, T3>>::deconvert(ps_tup); \encode
     */
    template <typename Indices = std::make_index_sequence<sizeof...(Ts)>>
...
private:
    ...
    static pubsub_type convert_impl(std::tuple<Ts...> tup,
                                    std::index_sequence<I...>)
    {
        return std::tuple_cat(
            convert_and_tuplify(std::move(std::get<I>(tup)))...);
    }
    ...
    static auto extract_and_deconvert(pubsub_type& tup)
    {
        ...
        return PubSubConverter<T>::deconvert(remove_tuple_if_single(
            extract_Nth_tuple<I, pubsub_type, Ts...>(tup)));
    }
...
struct PubSubConverter<std::unordered_map<T, S>>
{
    ...
    static pubsub_type convert(input_type in)
    {
        ...
        return PubSubConverter<before_final_t>::convert(std::make_tuple(
            PubSubConverter<std::vector<T>>::convert(keys_vec),
...
private:
    ...
    static PubSub_t<std::vector<S>> build_element_vector(input_type& input)
    {
        std::vector<S> to_ret;
        for (auto it : input)
            to_ret.push_back(std::get<ind>(it));
        return PubSubConverter<std::vector<S>>::convert(to_ret);
    }

    template <std::size_t... I>
    static pubsub_type convert_impl(input_type input, std::index_sequence<I...>)
    {
        before_final_t bf_tup = std::make_tuple(
            build_element_vector<std::tuple_element_t<I, tuple_type>, I>(
                input)...);
    ...
    // {
    //   return
    //   PubSubConverter<std::vector<S>>::deconvert(std::get<ind>(tup)).at(i);
    ...
    static input_type deconvert_impl(pubsub_type tup, std::index_sequence<I...>)
    {
        before_final_t bf_tup = PubSubConverter<before_final_t>::deconvert(tup);
        swapped_input_t swapped_to_ret = std::make_tuple(
        ...
        //   to_ret.push_back
        //     (std::make_tuple
        //      (get_vector_element<std::tuple_element_t<I, tuple_type>,
        //      I>(bf_tup, i)...));
        // }
        return to_ret;
    }
...
struct PubSubConverter<std::vector<std::vector<T>>>
{
    ...
    static input_type deconvert(pubsub_type input)
    {
        ...
        vecPsT_t flat_vec =
            PubSubConverter<vecPsT_t>::deconvert(std::get<1>(bf));
...
struct PubSubConverter<
    std::vector<T>,
    std::enable_if_t<!IsNativeToSkywing_v<std::vector<T>> && !IsTuple_v<T>
...
//   static pubsub_type convert(SirWalter sw) { return std::make_tuple(sw.a,
//   sw.b); } static SirWalter deconvert(pubsub_type in) { return
//   SirWalter{std::get<0>(in), std::get<1>(in)}; }
// };
