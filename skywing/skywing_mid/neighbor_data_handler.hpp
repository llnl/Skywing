...
public:
     ...
     *
     *  @param tags A reference to the tags of neighbor data to which we are
     * subscribed.
     *  @param neighbor_values A reference to the neighbor's values.
     *  @param updated_tags A reference to a vector of tags that have been
     * updated recently.
     *
     ...
     */
    NeighborDataHandler(
        std::function<DataType(const BaseDataType&)> transformer,
        ...
        : transformer_(transformer),
          tags_(tags),
          neighbor_values_(neighbor_values),
          updated_tags_(updated_tags)
    {}
    ...
    NeighborDataHandler<BaseDataType, SubDataType> get_sub_handler(
        std::function<SubDataType(const DataType&)> sub_transformer) const
    {
        return NeighborDataHandler<BaseDataType, SubDataType>(
            ...
            [=, *this]
#endif
            (const BaseDataType& v) {
                return sub_transformer(transformer_(v));
            },
            tags_,
            neighbor_values_,
            updated_tags_);
    ...
    // NeighborDataHandler<BaseDataType, typename std::tuple_element<index,
    // DataType>::type> get_tuple_element_handler() const
    // {
    //   return get_sub_handler<typename std::tuple_element<index,
    //   DataType>::type>
    ...
    R sum() const
    {
        return f_accumulate_<R>(transformer_, std::plus<R>());
    }
    ...
    R weighted_sum(tag_map<TagType, S>& coeffs) const
    {
        return weighted_f_accumulate_<DataType>(
            transformer_,
            [&](const TagType& t) { return coeffs[t]; },
            std::plus<R>());
    }
    ...
    {
        return weighted_f_accumulate_<R>(
            [&](const BaseDataType& v) { return f(transformer_(v)); },
            std::plus<R>());
    }
    ...
    {
        R num = sum();
        R denom = f_accumulate_<R>([](const BaseDataType&) { return 1.0; },
                                   std::plus<R>());
        return num / denom;
    }
     ...
     * @tparam R The return type, \c DataType by default.
     *
     * @param coeffs A map from tag to \c S, representing data
    ...
    R weighted_average(tag_map<TagType, S> coeffs) const
    {
        ...
        DataType denom = weighted_f_accumulate_<S>(
            [&](const TagType& t) { return coeffs[t]; }, std::plus<S>());
        return num / denom;
    }
     ...
     */
    const std::vector<const TagType*>& get_updated_tags() const
    {
        return updated_tags_;
    }
...
private:
    ...
    /** @brief Compute an affine accumulation of a function of the data, \f$s +
     * \sum_i c_i f(x_i)\f$.
     *
     ...
     * @param coef The coefficients, represented as a function from a \c TagType
     * to \c S.
     * @param binary_op The binary accumulation function, such as \c
     * std::plus<R>.
                             ...
                             R* shift) const
    {
        ...
        while (neighbor_values_.find(*tag_iter) == neighbor_values_.end())
            ++tag_iter;
        R val =
            binary_op(*shift, coef(*tag_iter) * f(neighbor_values_[*tag_iter]));

        ++tag_iter;
        for (; tag_iter != tags_.cend(); ++tag_iter) {
            if (neighbor_values_.find(*tag_iter) == neighbor_values_.end())
                continue;
            val =
                binary_op(std::move(val),
                          coef(*tag_iter) * f(neighbor_values_.at(*tag_iter)));
        }
        return val;
    }

    /** @brief Compute a linear accumulation of a function of the data,
     * \f$\sum_i c_i f(x_i)\f$.
     *
     * @tparam R The output type of the computation.
     * @tparam S The coefficient type \f$c_i\f$.
    ...
    /** @brief Compute an accumulation of a function of the data, \f$\sum_i
     * f(x_i)\f$.
     *
     * @tparam R The output type of the computation.
     *
