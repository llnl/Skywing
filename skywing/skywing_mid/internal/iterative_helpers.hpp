...
{
    using tuple_of_value_type =
        std::tuple<typename T::ValueType>; // typename
                                           // TupleIfNotAlready<typename
                                           // T::ValueType>::tuple_type;
    using tuple_of_type =
        std::tuple<T>; // typename TupleIfNotAlready<T>::tuple_type;
};

/** @brief Get std::tuple<T1::ValueType, T2::ValueType, ...> if all ValueTypes
 * are tuples.
 */
...
struct TupleOfValueTypes
{
    using type = decltype(std::tuple_cat(
        std::declval<typename IfHasValueType<Ts>::tuple_of_value_type>()...));
};
...
struct IndexOf<T, std::tuple<TOther, Ts...>>
{
    ...
    static constexpr std::size_t index =
        1 + IndexOf<T, std::tuple<Ts...>>::index;
};
...
{
    using type = decltype(std::tuple_cat(
        std::declval<typename IfHasValueType<Ts>::tuple_of_type>()...));
};
...
template <class T>
struct has_ValueType<T, std::void_t<typename T::ValueType>>
