...
struct IsPubSubType
{
    static constexpr bool value = (internal::index_of<T, PublishValueTypeList>
...
{
    return TupleIfNotAlready<PubSub_t<S>>::to_tuple(
        PubSubConverter<S>::convert(std::move(t)));
}
...
auto extract_Nth_tuple(PSTuple ps_tup)
{
    ...
    constexpr std::size_t num_preceding =
        I == 0 ? 0 : size_of_first_N_v<I - 1, Ts...>;
    using index_seq_for_Nth =
        offset_sequence_t<num_preceding, std::make_index_sequence<Nth_size>>;
 ...
 * std::tuple<PS21, PS22> =
 * decltype(remove_tuple_if_single(extract_Nth_tuple<1>(ps_tup))); PS3 =
 * decltype(remove_tuple_if_single(extract_Nth_tuple<2>(ps_tup))); \endcode
 */
