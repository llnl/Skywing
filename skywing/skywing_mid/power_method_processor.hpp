...
public:
    ...
    using SqNormSumProc_t = SumProcessor<scalar_t, PushFlowProcessor<double>>;
    // Values sent by this processor.
    using ValueType =
        std::tuple<AssocVec_t, typename SqNormSumProc_t::ValueType>;

    PowerMethodProcessor(AssocVec_t my_column, index_t my_index)
        : my_index_(my_index),
                        ...
                        const IterMethod& iter_method)
    {
        ...
        if (std::abs(eigenvalue_estimate_) > 1e-2)
            my_eigvec_element_ = 0.75 * my_eigvec_element_
                                 + 0.25 * (my_Ax / eigenvalue_estimate_);
