...
public:
                        ...
                        [[maybe_unused]] const IterMethod&)
    {
        for (const auto& pTag : nbr_data_handler.get_updated_tags()) {
            ...
            for (size_t nbr_vals_ind = 0; nbr_vals_ind < (nbr_value.size() / 2);
                 nbr_vals_ind++)
            {
                ...
                for (size_t row_index_cycle = 0;
                     row_index_cycle < number_of_updated_components_;
                     row_index_cycle++)
                {
                    if (nbr_value[nbr_vals_ind * 2]
                        == row_indices_[row_index_cycle])
                        use_this_value = false;
                }
                if (use_this_value) {
                    size_t updated_index =
                        static_cast<size_t>(nbr_value[nbr_vals_ind * 2]);
    ...
    // void process_update([[maybe_unused]] const std::vector<ValueTag>&
    // nbr_tags,
    ...
    //       // replacing its own updates if there's overlap in the
    //       // linear system partition.
    //       for(size_t row_index_cycle = 0 ; row_index_cycle <
    //       number_of_updated_components_; row_index_cycle++)
    //       {
    ...
    //       if(use_this_value)
    //       {
    //         size_t updated_index = static_cast<size_t>(nbr_value[nbr_vals_ind
    //         * 2]); x_iter_[updated_index] = nbr_value[nbr_vals_ind * 2 + 1];
    //         jacobi_computation();
    //       }
    //     }
    //   }
