...
template<typename ConcreteSynchronous, typename Range, typename... CSArgs>
auto create_synchronous_iterative(
  MasterHandle handle,
  Job& job,
  const typename ConcreteSynchronous::ValueTag& produced_tag,
  const Range& tags,
  CSArgs&&... args) noexcept
{
  job.declare_publication_intent(produced_tag);
  return internal::create_iterative<ConcreteSynchronous, Waiter<internal::MasterSubscribeIsDone, WaiterGetNoOp>>(
    job.subscribe_range(tags),
    handle,
    job,
    produced_tag,
    tags,
    std::forward<CSArgs>(args)...);
}
