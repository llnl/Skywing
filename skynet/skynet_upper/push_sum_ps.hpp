...
public:
  ...
  PushSumPs(int machine_number, int size_of_system, int number_of_neighbors, double starting_value, std::vector<ValueTag> tags_vector, skynet::Job job):
    ...
    job(job)
  {
    ...
    job.declare_publication_intent(tags_vector[machine_number]);
    job.publish(tags_vector, publish_values);
    for (int i = 0; i < size_of_system; i++)
    {
      if (i != machine_number)
      {
        subscribeToTag(job, tags_vector[i]);      
      }
    }
  };
  ...
  void create_iteration()
  {
    for(int i = 0 ; i < size_of_system; i++)
    {
      if(i != machine_number && job.has_data[tags_vector[i]] )
      {
        new_information_count += 1.0;
        std::vector<double> received_values = *job.get_waiter(tags_vector[i]).get();
        push_sum_computation(i,received_values);
      }
    }
...
// auto create_push_sum_ps(int machine_number, int size_of_system, int number_of_neighbors, double starting_value, std::vector<ValueTag> tags, Args&&... args) noexcept
// {
...
//      if (it) { return PushSumPs(machine_number, size_of_system, number_of_neighbors, starting_value, tags, *it);}
//      else    { return {}; }
//   });
// }
