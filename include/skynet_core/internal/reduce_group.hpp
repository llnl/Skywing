...
namespace skynet
{
  ...
  namespace internal
  {
    ...
    public:
      ...
      private:
        ...
        static void report_disconnection(ReduceGroupBase& g) noexcept
        {
          return g.report_disconnection();
        }
        static const ReduceGroupNeighbors& tag_neighbors(const ReduceGroupBase& g) noexcept
        {
          return g.tag_neighbors();
        }
        ...
        static void propagate_disconnection(
          ...
          ReductionDisconnectID id
        ) noexcept
        {
          g.propagate_disconnection(initiating_machine, id);
        }
        static const TagID& produced_tag(const ReduceGroupBase& g) noexcept
        {
          return g.produced_tag();
        }
        static const TagID& group_id(const ReduceGroupBase& g) noexcept
        {
          return g.group_id();
        }
      };
    ...
    protected:
      ...
      void add_data_index(std::size_t index, const gsl::span<const PublishValueVariant> value, const VersionID version) noexcept
      {
        assert(index < 3);
      ...
      virtual void do_add_data_index(std::size_t index, gsl::span<const PublishValueVariant> value, VersionID version) noexcept = 0;
  ...
  public:
    ...
    {
      static_assert(
        (... && std::is_convertible_v<ArgTypes, Ts>),
        "Reduce called with invalid parameters!"
      );
      return reduce_impl<false>(
        std::move(reduce_op),
        ValueType{static_cast<Ts>(std::forward<ArgTypes>(values))...}
      );
    }
    template<typename Callable, typename... ArgTypes>
    auto reduce(Callable reduce_op, const std::tuple<ArgTypes...>& values_tuple) noexcept
    {
      const auto apply_to = [&](const auto&... values) {
        return reduce(std::move(reduce_op), values...);
      };
    ...
    auto allreduce(Callable reduce_op, ArgTypes&&... values) noexcept
    {
      static_assert(
        (... && std::is_convertible_v<ArgTypes, Ts>),
        "Allreduce called with invalid parameters!"
      );
      return reduce_impl<true>(
        std::move(reduce_op),
        ValueType{static_cast<Ts>(std::forward<ArgTypes>(values))...}
      );
    }
    ...
    {
      const auto apply_to = [&](const auto&... values) {
        return allreduce(std::move(reduce_op), values...);
      };
      return std::apply(apply_to, values_tuple);
    }
  ...
  private:
    ...
    static auto make_wrapper(Callable reduce_op) noexcept
    {
      ...
      {
        static_assert(
          std::is_same_v<Callable, Callable>,
          "Invalid Callable used for reduce operation!"
        );
      }
    }
    ...
    void do_process_pending_reduce_ops() noexcept override
    {
      ...
      for (auto iter = pending_reduces_.begin(); iter != pending_reduces_.end(); iter = pending_reduces_.erase(iter))
      {
        ...
        if (returns_value_on_reduce())
        {
          ...
          {
            send_value_to_children(result_span, iter->required_version);
          }
        }
        else
        {
    ...
    std::vector<PublishValueVariant> make_variant_vector(ValueType val, std::index_sequence<Is...>) noexcept
    {
      ...
      {
        return std::vector<PublishValueVariant>{std::move(val)};
      }
      else
      {
    ...
    {
      for (auto& buf : data_buffers_) { buf.reset(); }
    }
    ...
    auto reduce_impl(
      Callable reduce_op,
      ValueOrTuple<Ts...> value
    ) noexcept
    {
      std::lock_guard lock{buffer_mutex_};
      const auto required_version = last_sent_version_ + 1;
      pending_reduces_.push_back({
        required_version,
        value,
        this->make_wrapper(std::move(reduce_op)),
        IsAllReduce
      });
      ...
      return internal::make_waiter(
        ...
        [this, required_version, conn_id]() noexcept {
          if (conn_id < conn_counter || !is_valid)
          {
            return true;
          }
          if constexpr (IsAllReduce)
          {
            return data_buffers_[0].has_data(required_version);
          }
          else
          {
            return
              last_sent_version_ != internal::tag_no_data &&
              last_sent_version_ >= required_version;
          }
        },
