...
requires *first* summing of a set of numbers, and *then*
...
*Note:* It is important to draw a distinction between the *local*
  ...
  any method of determining doneness for a collective operation.
...
```
The `IterateUntilTime` and `TrivialResiliencePolicy` types are defined by
...
publish during the iterative method, and the tag IDs to which this
agent will subscribe during the iterative method.

2. Call `set_procesor`, `set_iteration_policy`, and `set_resilience_policy`
...
int main(const int argc, const char* const argv[])
{
  ...
  auto pubsub_job = [&](Job& job, ManagerHandle manager_handle)
  {
    ...
    using IterMethod = SynchronousIterative
      <MeanMethod, IterateUntilTime, TrivialResiliencePolicy>;
    Waiter<IterMethod> iter_waiter =
...
The code in this file is built as `ex3` in the example CMake project
...
`run_ex3.sh`. We run this and get an output like the following:

```
$ ${TUTORIAL_HOME}/build-tutorial/run3.sh
./ex3 0
./ex3 1
./ex3 2
