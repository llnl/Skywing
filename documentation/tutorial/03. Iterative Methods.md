...
Skynet makes heavy use of C++'s templating capability. To better
...
Skynet solves most problems through iterative methods. In fact, even
problems that one normally thinks of as direct, such as computing a
mean, are typically solved in Skynet via iterative methods. The reason
...
dividing by the size of the set. To compute a mean in this way, Skynet
would need to somehow provide a guarantee that all terms in the sum
have been accounted for before dividing; given that Skynet collectives
...
methods, on the other hand, enable Skynet collectives to jointly
...
Skynet provides a flexible, template-based system for constructing
...
Currently, Skynet provides two iterative frameworks:
 * `SynchronousIterative` is a class template in which an agent waits until it receives each of its immediate neighbors' updates before applying its own. Available in `skynet/skynet_mid/synchronous_iterative.hpp`.
 * `AsynchronousIterative` is a class template that does not wait for all its neighbors, applying its own update method as fast as it desires with whatever data are currently available. Available in `skynet/skynet_mid/asynchronous_iterative.hpp`.
...
`skynet/skynet_mid/synchronous_iterative.hpp`. We will make use of an
averaging algorithm called Push Flow; the processor that implements
this is available in `skynet/skynet_mid/push_flow_processor.hpp`.
...
Skynet itself, and simply indicate that we will run for a given amount
...
`skynet/skynet_mid/synchronous_iterative.hpp`, there is also defined a
...
to the Skynet Manager, the job handle, the tag ID this agent will
...
```
g++ -std=c++17 ex3.cpp -o ex3 -I$(skynet_root)/skynet -I$(skynet_root)/subprojects/spdlog/include -I$(skynet_root)/subprojects/gsl/include -I$(skynet_root)/build/generated_files/generated `pkg-config capnp --cflags --libs` -L$(skynet_root)/build/skynet/skynet_core -lskynet_core
```

Like before, we will use a bash script to start up the processes
running the Skynet agents. This time we will use a slightly more
