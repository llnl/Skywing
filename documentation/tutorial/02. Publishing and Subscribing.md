...
In order for a Skynet collective to perform useful work, its operable
agents need to be able to share data and results of computations with
each other. But because Skynet is intended for situations with
...
Skynet as being based on pairs of agents sending messages back and
forth.
...
Frequently, a Skynet application is a streaming application, such as a
...
# A Skynet application with pubs and subs

## First, a core programming construct in Skynet
...
operation; furthermore, because Skynet deals with inherently
unreliable devices, an operation must be able to fail without it
causing a Skynet agent to crash.

To handle these requirements, Skynet provides the `Waiter<T>` class
...
`skynet/skynet_core/waiter.hpp`. We will introduce more about the
...
## A Skynet program that makes connections and communicates
...
```
using pubtag_t = skynet::PublishTag<int>;
...
requires that the Skynet Manager initiative a search protocol within
...
connection pattern! This shows how Skynet's underlying tag search
...
```
g++ -std=c++17 ex2.cpp -o ex2 -I$(skynet_root)/skynet -I$(skynet_root)/subprojects/spdlog/include -I$(skynet_root)/subprojects/gsl/include -I$(skynet_root)/build/generated_files/generated `pkg-config capnp --cflags --libs` -L$(skynet_root)/build/skynet/skynet_core -lskynet_core
```
