...
resilience in acquisition of computational and intermediate
...
Frequently, a Skywing application is a streaming application, such as
a streaming sense-and-control loop. Therefore, data made available to
...
published data in the collective is uniquely identified by its
**tag**, which is a string-based name of the tag. Although multiple
agents can publish under the same tag, the expectation is that they
are all publishing the same data. *That is, a data stream's tag does
not identify the agent(s) providing it (so more than one could publish
under it), but it does uniquely identify the data itself.*
...
wishes to subscribe to a data stream, the following sequence of operations is executed:
...
### Configuring initial connections

We configure the initial handshake connection asking the Manager to
`configure_initial_neighbors`, in which we pass the IP address and listening
port number of the neighboring agent to which we wish to connect.
The actual connection is made when a Job is run, using a function that returns
a `Waiter<bool>` that, upon `get`ing, returns a Boolean indicating whether or
not the connection was made. A connection can fail for a multitude of reasons,
including that the neighbor simply isn't ready yet, and so a false value
doesn't necessarily indicate a problem. Therefore, the
`configure_initial_neighbors` function takes an optional timeout input parameter
to dictate how long we should keep trying to make the connection.
Note that making the initial neighbor connection will block until the connection is made
OR the function times out, therefore the timeout parameter (10 seconds by default) should be
set judiciously so you have the correct behavior for your application.
...
requires that the Skywing Manager initiate a search protocol within
...
Recall that, as mentioned above, more than one agent can publish under
the same tag, with the expectation that any such duplication is from
agents publishing "the same" data. The completion of a subscription
promises that the subscribing agent has obtained access to _some_
agent publishing under the tag, but does not make any promises which
agent that is; the publishing and subscribing agent can even be the
same!
...
subscriptions even though it didn't initially connect to those. In
...
int main(const int argc, const char* const argv[])
{
  ...
  if (machine_num > 0)
    manager.configure_initial_neighbors("localhost", ports[machine_num - 1]);
...
The code in this file is built as `ex2` in the example CMake project
described in [part 1](01.\ Introduction.md).
...
automatically kills them after a while. This is to ensure that, in case
...
This shell script is generated by the example CMake project as
`run_ex2.sh`. We run this and get an output like the following:

```
$ ${TUTORIAL_HOME}/build-tutorial/run2.sh
./ex2 0
./ex2 1
./ex2 2
