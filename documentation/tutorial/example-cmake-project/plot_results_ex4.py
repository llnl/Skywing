...
for agent, arrays in output_dicts["TotalChargeEstimate"].items():
    time_array, values_array = zip(*arrays, strict=True)
...
time_min = min(
    [arrays[0][0] for agent, arrays in output_dicts["BatteryChargeSensor"].items()]
)
time_max = min(
    [arrays[-1][0] for agent, arrays in output_dicts["BatteryChargeSensor"].items()]
)
time_common = np.linspace(time_min, time_max, 200)
sensor_common = False
for _agent, arrays in output_dicts["BatteryChargeSensor"].items():
    time_array, values_array = zip(*arrays, strict=True)
    interp = interp1d(
        time_array, values_array, bounds_error=False, fill_value="extrapolate"
    )
...
for agent, arrays in output_dicts["BatteryChargeSensor"].items():
    time_array, sensor_array = zip(*arrays, strict=True)
...
for agent, arrays in output_dicts["ReceivedTotalCharge"].items():
    time_array, state_array = zip(*arrays, strict=True)
...
for agent, arrays in output_dicts["Flow"].items():
    time_array, alert_array = zip(*arrays, strict=True)
