...
collective at startup; through connections and collective searches,
...
handle to the `Manager` object. For now, let's create a Job that doesn't do
anything:
...
this case just prints a message and returns) and then asks the
`manager` to run all its submitted `job`s.
...
Now let's compile and run it. Skywing requires C++20 and a few include
 ...
 * Path to *generated* cap 'n' proto includes, included in the Skywing
...
Conveniently, these are all captured in the CMake project packaged
with this tutorial. This builds as any standard CMake project; see
[BUILD.md](BUILD.md) for simple instructions.
...
A script to this effect is generated by the example CMake project with
the name `run_ex1.sh`. Assuming the example project has been built as
in [BUILD.md](BUILD.md), we can run this as follows and should see
output like the following:

```
$ ${TUTORIAL_HOME}/build-tutorial/run.sh
...
any connections with other agents. In our
[next tutorial](02.\ Publishing\ and\ Subscribing.md), we will look into how agents
communicate.
