...
Skynet is a library designed to provide tools for a user to build
...
their program in terms of these Skynet building blocks, their program
inherits these resiliency properties automatically.

For example, if a user needs to compute a summation over a group of
devices and does through via Skynet' reduce function, which
...
# Basic Concepts in Skynet

An individual device running software using Skynet is called an
**agent**. A collection of Skynet agents working together is called a
...
Although Skynet is designed for the use case in which agents are
spread out across an edge-based computing network, this is not
actually a requirement to run a Skynet collective. In fact, for
...
Skynet agents *do not* need to know about all other agents in the
...
Once initial connections are made, Skynet agents communicate via a
...
## Skynet Managers and Jobs

The Skynet software is designed to enable highly asynchronous
workflows. To that end, each Skynet agent runs a **Manager** thread which
...
the Skynet user. It will declare the IP and port of any initially
...
Job will also leverage the Skynet user library to request Skynet
objects that can perform reduces, iterative methods, and other
collective operations.

# Your First Skynet Program

## Building Skynet

Build instructions can be found in the [root readme file](/README.md).

## My first Skynet program

Let's write a Skynet program that establishes a connection between two
agents and sends some data back and forth. We will go through this
step by step.

First, we will `#include` the primary Skynet header file, and we will
...
it. To do this, we must provide the Skynet Manager with a *function*
...
```
  ...
  {
    std::cout << "Hello Skynet world!" << std::endl;
  });
  manager.run();
```
...
Ta da! We have our first Skynet progam. Altogether, it looks like this:
...
int main(const int argc, const char* const argv[])
{
  ...
  manager.submit_job("job_name", [&](Job& job, ManagerHandle manager_handle)
  {
    std::cout << "Hello Skynet world from agent " << machine_name << "!" << std::endl;
  });
  manager.run();
}
```
...
Now let's compile and run it. Skynet requires C++17 and a few include
directories. Here's what we need to include:
 * Path to skynet includes
 * Path to spdlog includes (for logging), in the subprojects folder
 * Path to gsl, in the subprojects folder
 * Path to cap 'n' proto includes, unmanaged by Skynet, but if you successfully built Skynet you have this. Often accessible through pkg-config
 * Path to *generated* cap 'n' proto includes, in the Skynet build directory

And we need to link to the following:
 * The cap 'n' proto library
 * The `skynet_core` library

For example, a typical build command might look like the following:
```
g++ -std=c++17 ex1.cpp -o ex1 -I$(skynet_root)/skynet -I$(skynet_root)/subprojects/spdlog/include -I$(skynet_root)/subprojects/gsl/include -I$(skynet_root)/build/generated_files/generated `pkg-config capnp --cflags --libs` -L$(skynet_root)/build/skynet/skynet_core -lskynet_core
```
...
line, but recall that Skynet is intended to be a parallel system
...
```
$ source run.sh
Hello Skynet world from agent node0!
Hello Skynet world from agent node1!
