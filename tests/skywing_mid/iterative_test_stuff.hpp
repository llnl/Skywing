...
#include <stddef.h> // size_t, not std::size_t.
...
struct TestWaitForNbrsStopPolicy
{
    ...
    ValueType
    prepare_for_publication([[maybe_unused]] ValueType vals_to_publish)
    {
        return get_curr_val();
    }
...
public:
                        ...
                        [[maybe_unused]] const IterMethod&)
    {
        if (nbr_data_handler.num_neighbors() != num_machines_) {
            std::cerr << "Machine " << machine_ind_ << " expected "
                      << num_machines_ << " neighbors but only have "
