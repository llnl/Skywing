...
void server()
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        while (with_client.read_message(int_buffer.data(), int_buffer.size())
...
void client()
{
    const auto do_send = [](SocketCommunicator& conn) {
        ...
        std::lock_guard<std::mutex> lock{catch_mutex};
        REQUIRE(conn.send_message(int_buffer.data(), int_buffer.size())
                == ConnectionError::no_error);
    };
    // Blocking
    {
    ...
    // Non-blocking
    {
        ...
        [&]() {
            while (true) {
                ...
                case ConnectionError::no_error: return;

                default:
                    std::cerr << "Error returned from "
                                 "conn.connection_progress_status()"
                              << std::endl;
                    std::exit(1);
                    break;
                }
