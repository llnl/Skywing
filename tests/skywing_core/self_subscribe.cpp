...
TEST_CASE("Self-subscription works", "[core]")
{
    ...
    REQUIRE(private_pub_fut.get() == 30);
        ...
        // is a non-trivial addition const GroupTag reduce_tag{"self_reduce"};
        // auto group_fut1 = job.create_reduce_group(reduce_tag, tags[0],
        // {tags.begin(), tags.end()}); auto group_fut2 =
        // job.create_reduce_group(reduce_tag, tags[1], {tags.begin(),
        // tags.end()}); REQUIRE(group_fut1.wait_for(wait_time));
