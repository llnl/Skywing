...
using Int64Tag = Tag<std::int64_t>;
const Int64Tag value_tag{"value"};

std::atomic<int> values_retrieved = 0;

std::uint16_t subscriber_start_port()
{
    return get_starting_port();
}
std::uint16_t publisher_start_port()
{
    return get_starting_port() + 1;
}
...
void subscriber()
{
    ...
    base_manager.submit_job("job", [&](Job& job, ManagerHandle) {
        ...
        while (values_retrieved != num_values_to_publish) {
            std::cout << "Subscriber has received " << values_retrieved
                      << " values" << std::endl;
            if (values_retrieved != 0) {
                // wait a bit so the publisher can disconnect
                std::this_thread::sleep_for(std::chrono::milliseconds(10));
                std::cout
                    << "Subscriber about to rebuild missing tag connections"
                    << std::endl;
                job.rebuild_missing_tag_connections().wait();
                REQUIRE(0 == 1);
                std::cout
                    << "Subscriber finished rebuilding missing tag connections"
                    << std::endl;
            }
            ...
            std::cout << "Subscriber about to get another value from the same "
                         "publisher"
                      << std::endl;
            const auto failed_value = job.get_waiter(value_tag).get();
            std::cout << "Subscriber finished attempting to get another value "
                         "from same publisher"
                      << std::endl;
