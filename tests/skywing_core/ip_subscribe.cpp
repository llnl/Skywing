...
std::uint16_t subscriber_port()
{
    return get_starting_port();
}
std::uint16_t publisher_port()
{
    return subscriber_port() + 1;
}
...
void subscriber()
{
    ...
    manager.submit_job("subscriber", [&](Job& job, ManagerHandle) {
        ...
        job.ip_subscribe("127.0.0.1:" + std::to_string(publisher_port()), tag)
            .wait();
...
TEST_CASE("Subscribe to specific IP works", "[core]")
{
