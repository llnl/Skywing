...
//This reads in the appropriate entries of b that a specific process needs from a matrix market format.
std::vector<double> obtain_local_solution_vector(int size_of_system, std::vector<int> row_indices, std::string sol_file_name, std::string root_pathway, int lc)
{
  ...
  {
    filePath = root_pathway + "/skynet/examples/include/system_hold_folder/sol_hold_folder/" + sol_file_name;
  }
  // std::string filePath "/g/g20/aaronb/test_redundant_parallel_jacobi/matrix_hold_folder/"+rhs_file_name_name;
...
std::vector<double> obtain_full_solution_vector(int size_of_system, std::string sol_file_name, std::string root_pathway, int lc)
{
