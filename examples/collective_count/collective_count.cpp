                  ...
                  std::vector<std::string> tagIDs)
{
    ...
    manager.submit_job(
        "job", [&](skywing::Job& job, ManagerHandle manager_handle) {
            ...
            iter_method.run([&](const IterMethod& p) {
                std::cout << p.run_time().count() << "ms: Machine "
                          << machine_number << " has count "
                          << p.get_processor().get_count() << ", min "
                          << p.get_processor().get_min() << ", mean "
                          << p.get_processor().get_mean() << std::endl;
            });
