                       ...
                       ParamStruct const& param_struct)
{
    ...
    if (agent_number != static_cast<size_t>((ports.size()) - 1)) {
        ...
        while (!manager_handle
                    .connect_to_server("127.0.0.1", ports[agent_number + 1])
                    .get())
        {}
    }
    ...
    auto update_fun = [&](IterMethod& p) {
        ...
        std::cout << p.run_time().count() << "ms: Agent " << agent_number
                  << " producing value " << current_value << std::endl;
                          ...
                          ParamStruct const& param_struct)
{
    ...
    for (size_t i = 0; i < 5; i++) {
        for (size_t j = 0; j < 4; j++) {
            ...
            if (curr_sum_result) {
                std::cout << "Agent " << agent_number
                          << " seeing summation result " << *curr_sum_result
                          << std::endl;
                std::this_thread::sleep_for(std::chrono::seconds(3));
            }
        }
        contrib_value = contrib_value * 10;
        std::cout << "Agent " << agent_number << " increasing contrib to "
