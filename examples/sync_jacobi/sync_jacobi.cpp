...
void machine_task(const int machine_number,
                  int trial,
    ...
    manager.submit_job(
        "job", [&](skywing::Job& job, ManagerHandle manager_handle) {
            ...
            auto x_local_estimate =
                sync_jacobi.get_processor().return_full_solution();
            auto x_partition_estimate =
                sync_jacobi.get_processor().return_partition_solution();
...
int main(int argc, char* argv[])
{
    ...
    std::string row_index_name = "machine_" + std::to_string(machine_number)
                                 + "_row_count_" + std::to_string(0)
                                 + "_indices_" + matrix_name;
    std::vector<size_t> row_indices =
    ...
    std::string matrix_partition_name =
        ...
        + std::to_string(0) + "_" + matrix_name;
    // std::vector<double> matrix_row_hold =
    ...
    std::string rhs_partition_name = "machine_" + std::to_string(machine_number)
                                     + "_row_count_" + std::to_string(0)
                                     + "_rhs_" + matrix_name;
    std::vector<double> b_partition =
        input_vector_from_matrix_market<double>(directory, rhs_partition_name);

    std::string x_sol_partition_name =
        "machine_" + std::to_string(machine_number) + "_row_count_"
        + std::to_string(0) + "_x_sol_" + matrix_name;
    std::vector<double> x_partition_solution =
        input_vector_from_matrix_market<double>(directory,
                                                x_sol_partition_name);
    ...
    std::vector<double> x_full_solution =
        input_vector_from_matrix_market<double>(
            "../../../examples/sync_jacobi/system", x_sol_name);
