...
constexpr std::array<const char*, 2> node_names{
  "node1", "node2
}; //constexpr means that the computation is done at compile time rather than at run time.
...
constexpr std::array<std::uint16_t, node_names.size()> node_ports{
  10000, 11000
}; //std::uint16_t is a fixed bit declared for an unsigned integer of exactly 16 bits

// Tag that can be used to send a value for a reduce operation
// This tag uses a signed 32-bit integer for the value to send
using I32ValueTag = skynet::ReduceValueTag<std::int32_t>; // using here means to declare an alias
...
// This is the lambda function type implementation.
void simulate_machine(const int machine_number)
{
  ...
  master.submit_job("job", [&](skynet::Job& job, skynet::MasterHandle master_handle) /**/
  {
    ...
    //auto keyword deduces function return type, useful if derived types are large and cumbersome to write (easy to change types if necessary.)
    ...
    // uniform_int_distribution selects integer values uniformly form a given input range.
    ...
    // ranlux48 is subtract-with-carry pseudo-random generator of 48-bit numbers with accelerarted advancement. ???
    std::ranlux48 prng{std::random_device{}()};
    // these values are to later confirm that the result is indeed a ``sane'' result.
    ...
    while (true)
    {
      ...
      // plus is a function for performing addition, which notably can be applied to an entire array.
      // moreover, allreduce serves as a small meta example for how further functions are defined; that is, all of this code past the /**/ to the end is what the user needs to adjust in order to implement new functions. For this example, allreduce(callable function, random_value) is where the action is. The rest of the code is merely checks on this, along with an error message if a value within expected parameter is output, and the expected value along with the current date and time.
...
// This is the example without the lambda function parameter that controls the action.
// This is taken from Aly's source code average_demo.hpp from the cs-derms.
void simulate_machine(skynet::Job& job, skynet::MasterHandle masterHandle)
{

  auto reduce_group_waiter = job.create_reduce_group(I32GroupTag{"number exchange"}, reduce_group_tags[machine_number], reduce_group_tags);
...
int main(const int argc, const char* const argv[])
{
  ...
  // Lambda function type implementation
  simulate_machine(machine_number);


  // Job-Master type implementation
  simulate_machine(job, masterHandle);
