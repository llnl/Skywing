        ...
        "job", [&](skywing::Job& job, ManagerHandle manager_handle) {
            ...
            Waiter<IterMethod> iter_waiter =
                WaiterBuilder<IterMethod>(
                    ...
                    .build_waiter();
            std::cout << "Machine " << machine_number
                      << " about to get iteration object." << std::endl;
            IterMethod async_jacobi = iter_waiter.get();

            std::cout << "Machine " << machine_number
                      << " about to start jacobi iteration." << std::endl;
            async_jacobi.run([&](const decltype(async_jacobi)& p) {
                std::cout << p.run_time().count() << "ms: Machine "
                          << machine_number << " has values ";
                print_vec<double>(
                    p.get_processor().return_partition_solution());
            });
            std::cout << "Machine " << machine_number
                      << " finished jacobi iteration." << std::endl;

            double run_time = async_jacobi.run_time().count();
            int information_received = async_jacobi.get_iteration_count();
            auto x_local_estimate =
                async_jacobi.get_processor().return_full_solution();
            auto x_partition_estimate =
                async_jacobi.get_processor().return_partition_solution();
            // Since this is a distributed algorithm, we only have access to
            // information that allows us to have a "partial" residual, since
            // not every agent has every row of the matrix. In contrast, we can
            // look at error involving only the components of the solution
            // vector x which this process updates, or it's entire estimation
            // vector, hence "partial" versus "full" in this language and "PSQ"
            // versus "FSQ" for "partial error squared" and "full error
            // squared". We avoid taking square roots here in case additional
            // post processing is wanted.
            double partial_residual = calculate_partial_residual(
                x_local_estimate, b_partition, A_partition);
            double partial_forward_error = calculate_partial_forward_error(
                row_indices, x_partition_estimate, x_partition_solution);
            double forward_error = calculate_local_forward_error(
                x_local_estimate, x_full_solution);

            // Saves information from each Skywing machine for post processing,
            // if wanted.
            collect_data_each_component(machine_number,
                                        number_of_overlapping_components,
                                        trial,
                                        partial_forward_error,
                                        partial_residual,
...
int main(int argc, char* argv[])
{
    ...
    // Parse the machine number, starting_port_number, and size_of_network that
    // was passed in Do this in a lambda so that if there's an exception a dummy
    // value can be returned which will always trigger an error
    int machine_number = [&]() {
        try {
    ...
    if (size_of_network <= 0) {
        std::cerr << "Invalid size_of_network of " << std::quoted(argv[3])
                  << ".\n"
    ...
    if (matrix_name == "") {
        std::cerr << "Linear system not specified. " << std::quoted(argv[4])
                  << ".\n";
        return -1;
    }
    ...
    std::string row_index_name =
        "machine_" + std::to_string(machine_number) + "_row_count_"
        + std::to_string(number_of_overlapping_components) + "_indices_"
        + matrix_name;
    std::vector<size_t> row_indices =
        input_vector_from_matrix_market<size_t>(directory, row_index_name);

    std::string matrix_partition_name =
        "machine_" + std::to_string(machine_number) + "_row_count_"
        + std::to_string(number_of_overlapping_components) + "_" + matrix_name;
    std::vector<std::vector<double>> A_partition =
        input_matrix_from_matrix_market<double>(directory,
                                                matrix_partition_name);

    std::string rhs_partition_name =
        "machine_" + std::to_string(machine_number) + "_row_count_"
        + std::to_string(number_of_overlapping_components) + "_rhs_"
        + matrix_name;
    std::vector<double> b_partition =
        input_vector_from_matrix_market<double>(directory, rhs_partition_name);

    // Both of these vectors are contrived since we know the solution.
    // These are not used in the jacobi class, only for data output and terminal
    // diagnostics in the example.
    std::string x_partition_solution_name =
        "machine_" + std::to_string(machine_number) + "_row_count_"
        + std::to_string(number_of_overlapping_components) + "_x_sol_"
        + matrix_name;
    std::vector<double> x_partition_solution =
        input_vector_from_matrix_market<double>(directory,
                                                x_partition_solution_name);

    std::string x_sol_name = "x_sol_" + matrix_name;
    std::vector<double> x_full_solution =
        input_vector_from_matrix_market<double>(
            "../../../examples/async_jacobi/system", x_sol_name);
