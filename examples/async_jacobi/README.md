...
## Summary 
This is example showcases the Asynchronous Jacobi Method implemented in the Skynet upper for solving a system of linear equations, name, solving for x in Ax=b.
...
Unlike most implementations, this implementation is decentralized, so this does not follow the manager-slave model commonly found in parallel computing, so the "indexing" of the components passed between Skynet agents has to be explicit rather than some implicit partitioning scheme.
...
The naming convention used in the example for data input for each Skynet agent can be obtained by observing async_jacobi.cpp, and the accompanying python preprocessing script named jacobi_pre_processing.py.
