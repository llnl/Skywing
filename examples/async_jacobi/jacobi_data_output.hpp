double calculate_partial_residual(std::vector<double> x_local_solution,
                                  ...
                                  std::vector<std::vector<double>> A_partition)
{
    ...
    for (int i = 0; i < static_cast<double>(A_partition.size()); i++) {
        ...
        for (int j = 0; j < static_cast<double>(A_partition[0].size()); j++) {
            partial_residual_hold =
                A_partition[i][j] * x_local_solution[j] + partial_residual_hold;
        }
...
double calculate_partial_forward_error(std::vector<size_t> row_indices,
                                       std::vector<double> x_partition_estimate,
                                       std::vector<double> x_local_solution)
{
    double partitioned_forward_error = 0.0;
    // std::vector<double>
    // forward_error_vector(static_cast<int>(x_local_solution.size()),0.0);
...
double calculate_local_forward_error(std::vector<double> x_estimate,
...
void collect_data_each_component(int machine_number,
                                 int redundancy,
                                 int trial,
                                 double partial_forward_error,
                                 double partial_residual,
                                 int iteration_count,
                                 double time,
                                 std::string save_folder)
{
    std::string local_information_filename =
        save_folder + "/redundancy_" + std::to_string(redundancy) + "_trial_"
        + std::to_string(trial) + "_rank_" + std::to_string(machine_number)
        + ".csv";
