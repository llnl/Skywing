...
size_of_system = int(sys.argv[2])
name_of_system = sys.argv[3]
system_hold_folder = sys.argv[4]
partition_hold_folder = sys.argv[5]
...
for machine_number in range(0,size_of_system):
    ...
    for row in range(1,row_count):
        if((machine_number+row)<size_of_system):
            row_hold = np.vstack((row_hold,A[(machine_number+row),:]))
        else:
            row_hold = np.vstack((row_hold,A[(machine_number+row)%size_of_system,:]))
    if row_count==1:
        row_hold=row_hold[None,:]
        # row_hold=np.transpose(row_hold)
...
for machine_number in range(0,size_of_system):
    ...
    for row in range(1,row_count):
        if((machine_number+row)<size_of_system):
            row_hold=np.vstack((row_hold,b[(machine_number+row)]))
        else:
            row_hold=np.vstack((row_hold,b[(machine_number+row)%size_of_system]))
    if row_count==1:
        row_hold=row_hold[:,None]
...
for machine_number in range(0,size_of_system):
    ...
    for row in range(1,row_count):
        if((machine_number+row)<size_of_system):
            row_hold=np.vstack((row_hold,x_sol[(machine_number+row)]))
        else:
            row_hold=np.vstack((row_hold,x_sol[(machine_number+row)%size_of_system]))
    if row_count==1:
        row_hold=row_hold[:,None]
...
for machine_number in range(0,size_of_system):
    ...
    for row in range(1,row_count):
        if((machine_number+row)<size_of_system):
            row_hold=np.vstack((row_hold,indices[(machine_number+row)]))
        else:
            row_hold=np.vstack((row_hold,indices[(machine_number+row)%size_of_system]))
    if row_count==1:
        row_hold=row_hold[:,None]
