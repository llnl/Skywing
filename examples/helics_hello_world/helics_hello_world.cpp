...
void simulate_agent(const char* name, std::uint16_t local_port,
  std::vector<std::uint16_t>& remote_ports, DataTag skynet_publication,
  std::vector<DataTag>& skynet_subscriptions)
{
  ...
  master.submit_job("job",
    [&](skynet::Job& job, skynet::MasterHandle master_handle)
    {
      ...
      helics_federate value_fed =
        helicsCreateValueFederate(fed_name, fed_info, &err);
      if (err.error_code != helics_ok)
        std::exit(-3);
      // free memory
      ...
      helics_publication helics_pub =
        helicsFederateRegisterGlobalPublication(value_fed,
          skynet_publication.id().c_str(), helics_data_type_int, NULL, &err);
      if (err.error_code != helics_ok)
        std::exit(-4);
      ...
      for (auto& skynet_subscription : skynet_subscriptions)
      {
        helics_subscriptions.push_back(
          helicsFederateRegisterSubscription(value_fed,
            skynet_subscription.id().c_str(), NULL, &err));
        if (err.error_code != helics_ok)
          std::exit(-5);
      }
      ...
      if (err.error_code != helics_ok)
        std::cerr << "HELICS failed to enter initialization mode: "
          << err.message << std::endl;
      helicsFederateEnterExecutingMode(value_fed, &err);
      if (err.error_code != helics_ok)
        std::cerr << "HELICS failed to enter execution mode: "
          << err.message << std::endl;
      ...
      if (err.error_code != helics_ok)
        std::cerr << "HELICS request time failed: "
          << err.message << std::endl;
      ...
      for (auto& skynet_subscription : skynet_subscriptions)
      {
        if (job.tag_has_subscription(skynet_subscription))
          skynet_values[skynet_subscription.id()] =
            *job.get_waiter(skynet_subscription).get();
      }
      ...
      for (auto& helics_value : helics_values)
        std::cout << helics_value.first << ": " << helics_value.second
          << std::endl;
      std::cout << std::endl;
      std::cout << "Skynet values" << std::endl;
      for (auto& skynet_value : skynet_values)
        std::cout << skynet_value.first << ": " << skynet_value.second
          << std::endl;
      std::cout << std::endl;
...
int main(const int argc, const char* const argv[])
{
  ...
  simulate_agent(name, local_port, remote_ports, publication_tag,
    subscription_tags);
}
