...
An example is provided where Skynet agents using [HELICS](https://helics.readthedocs.io/en/latest/) to pass messages.
...
In this example, each Skynet agent generates a random number and then sends that number to every other Skynet agent through both Skynet and HELICS pub/sub mechanisms.
...
## Things to note
 ...
 1. The executable must be ran with two arguments
   - the number of Skynet agents to include
