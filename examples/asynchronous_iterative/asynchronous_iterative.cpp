...
using DataTag = skywing::Tag<double>;
...
// Callable signature: auto func(const double&, std::vector<double>&) ->
// std::pair<double, bool> The bool is to indicate if the function should
// continue iterating
    ...
    manager.submit_job(
        "job", [&](skywing::Job& job, skywing::ManagerHandle manager_handle) {
            ...
            if (!fut.wait_for(std::chrono::seconds(10))) {
                std::cerr << config.name
                          << ": Took too long to subscribe to tags\n";
                std::exit(1);
            }
            ...
            while (true) {
                ...
                for (const auto& sub_tag : config.tags_to_subscribe_to) {
                    if (job.has_data(sub_tag)) {
                        neighbor_values[sub_tag.id()] =
                            *job.get_waiter(sub_tag).get();
                    }
                }
                // Only call the function if there's any data that's been seen
                if (neighbor_values.empty()) {
                    // No values seen - check if all subscriptions are gone and
                    // exit if so
                    ...
                    std::tie(own_value, should_exit) =
                        act_on(own_value, other_values);
                    job.publish(config.tags_produced.front(), own_value);
                    if (should_exit) {
                        break;
                    }
                ...
                const auto sleep_ms =
                    std::uniform_int_distribution<int>{1, 5}(prng);
...
int main(const int argc, const char* const argv[])
{
    ...
    if (config_iter == configurations.cend()) {
        std::cerr << "Could not find configuration for machine \""
                  << machine_name << "\"\n";
        return 1;
    }
    ...
    asynchronous_iterative(
        ...
        [iter = 0](const double& self_value,
                   const std::vector<double>& other_values) mutable {
            constexpr int num_iters = 5'000;
            const auto new_value = std::accumulate(other_values.cbegin(),
                                                   other_values.cend(),
                                                   self_value)
                                   / (other_values.size() + 1);
            ++iter;
