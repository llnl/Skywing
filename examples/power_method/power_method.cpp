                  ...
                  std::vector<std::string> tagIDs)
{
    ...
    manager.submit_job(
        "job", [&](skywing::Job& job, ManagerHandle manager_handle) {
            ...
            // MyAssocVec matrix_column = [&]()
            //   {
            //     if (i == 0) return MyAssocVec({{0, 0.1}, {1, 1}, {2, 0}, {3,
            //     0}}); if (i == 1) return MyAssocVec({{0, 1}, {1, 0.1}, {2,
            //     1}, {3, 0}}); if (i == 2) return MyAssocVec({{0, 0}, {1, 1},
            //     {2, 0.1}, {3, 1}}); else return MyAssocVec({{0, 0}, {1, 0},
            //     {2, 1}, {3, 0.1}});
            //   }();
            ...
            std::vector<std::string> tagIDs_for_sub = [&]() {
                if (i == 0)
                    return std::vector<std::string>(
                        {tagIDs[i], tagIDs[i + 1], tagIDs[sos - 1]});
                else if (i == sos - 1)
                    return std::vector<std::string>(
                        {tagIDs[0], tagIDs[i - 1], tagIDs[i]});
                else
                    return std::vector<std::string>(
                        {tagIDs[i - 1], tagIDs[i], tagIDs[i + 1]});
            }();
            ...
            mean_or_sum.run([&](const decltype(mean_or_sum)& p) {
                std::cout << p.run_time().count() << "ms: Machine "
                          << machine_number << " has eigenvalue "
                          << p.get_processor().get_eigenvalue()
                          << " and eigenvector element "
                          << p.get_processor().get_eigenvector_element()
                          << std::endl;
            });
