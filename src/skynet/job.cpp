...
namespace skynet
{
  ...
  {
    const auto loc = expected_types_.find(tag_id);
    // Not subscribed; don't do anything, but not an error
    if (loc == expected_types_.cend()) { return true; }
    ...
    values_.insert_or_assign(tag_id, std::move(data));
    return true;
  }
  ...
  {
    // Start by having the master check for messages
    Master::Accessor::handle_neighbor_message(*master_);
    // Then check if there's been anything seen on the tag and return it if so
    if (const auto loc = values_.find(tag_id); loc != values_.cend())
    {
      return loc->second;
    }
    return {};
  ...
  bool Job::has_data_impl(const TagID& tag_id) noexcept
  {
    ...
    return (values_.find(tag_id) != values_.cend());
  }
  ...
  void Job::unsubscribe_impl(const TagID& tag_id) noexcept
  {
    ...
    expected_types_.erase(tag_id);
    values_.erase(tag_id);
  }
} // namespace skynet
