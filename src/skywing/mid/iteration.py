...
class Iteration:
    ...
    def update_data(self, *args) -> None:
        ...
        self.processor._update_data(*args)

    def update_parameters(self, **kwargs) -> None:
        """Update algorithm-specific parameters."""
        self.processor._update_parameters(**kwargs)

    def query(self):
        """Get the current result of the iteration."""
        return self.processor._query()
    ...
    def publish_data(self) -> None:
        ...
        """
        data = self.processor._prepare_for_publication()
        self.job.publish(self.my_tag, data)
    ...
    def gather_data(self) -> None:
        ...
        for tag in self.other_tags:
            if self.job.has_data(tag):
                ...
                self.iter_data[tag] = tag_data
                if __debug__:
                    if tag not in self.num_recvs:
                        self.num_recvs[tag] = 1
                    else:
                        self.num_recvs[tag] += 1
    ...
    def run(self, job) -> None:
        ...
        Args:
            job: Job instance providing publish/subscribe methods
        """
        self.job = job
        ...
        while not self.stop_policy.should_stop(self.processor):
            self.publish_data()
            self.gather_data()
            self.processor._process_update(self.my_tag, self.iter_data)
            time.sleep(0.0001)
